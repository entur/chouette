...
public abstract class 	GenericDAOImpl<T> implements GenericDAO<T> {
	...
	private List<T> findByObjectId(final Collection<String> objectIds, boolean flush) {
		...
		// using findByObjectId(String objectId) is more efficient than running a Criteria query
		// since it uses naturalId mapping and can retrieve an object directly from the persistence context
		// without hitting the database nor flushing the session.
		if(objectIds.size() == 1) {
